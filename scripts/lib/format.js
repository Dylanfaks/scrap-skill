...
// Monto en pesos, sin decimales: 118500 -> "$118.500"
const fmtPrice = (n) =>
  n == null ? "—" : "$" + Math.round(Number(n)).toLocaleString("es-AR");

// Monto compacto en millones: 7647000 -> "$7,65 M"
const fmtMoney = (n) =>
  "$" + (Number(n) / 1_000_000).toLocaleString("es-AR", { maximumFractionDigits: 2 }) + " M";
...
// Fecha+hora legible en es-AR para portadas. Recibe un Date.
const fmtDateTime = (d) =>
...
module.exports = { fmtInt, fmtPrice, fmtMoney, cleanShort, esc, truncate, fmtDateTime, slug, stamp };
